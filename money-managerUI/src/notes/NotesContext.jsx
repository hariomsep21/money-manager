...
import { getMonthNotes, setMonthNotes, listMonthKeys } from './storage/indexedDB';
...
function getMonthKeyFromDate(d) {
  return `${d.getFullYear()}-${d.getMonth()}`; // month 0-11
}
    ...
    (async () => {
      try {
        ...
        const entries = await Promise.all(
          (keys || []).map(async (k) => [k, await getMonthNotes(k)])
        );
        const initial = {};
        for (const [k, arr] of entries) {
          initial[k] = Array.isArray(arr) ? arr : [];
        }
